...
<body class="bg-wadimakkah-bg dark:bg-gray-900 min-h-screen flex flex-col transition-colors duration-200">
    ...
    <header class="bg-wadimakkah-dark text-white shadow-lg flex-shrink-0">
        <div class="px-16 py-6 flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex gap-8 text-sm font-medium">
                <a href="{{ route('manager.dashboard') }}" class="hover:text-wadimakkah-light transition text-white">الرئيسية</a>
                <a href="{{ Route::has('manager.cases') ? route('manager.cases') : '#' }}" class="hover:text-wadimakkah-light transition text-white">القضايا</a>
                <a href="{{ Route::has('manager.contracts') ? route('manager.contracts') : '#' }}" class="hover:text-wadimakkah-light transition text-white">العقود</a>
                <a href="{{ route('manager.tasks') }}" class="hover:text-wadimakkah-light transition text-white">إسناد المهام</a>
                <a href="{{ route('manager.consultations.incoming') }}" class="hover:text-wadimakkah-light transition text-white">طلبات الاعتماد</a>
                <a href="{{ Route::has('manager.orders.records') ? route('manager.orders.records') : (Route::has('manager.records') ? route('manager.records') : '#') }}" class="hover:text-wadimakkah-light transition text-white">السجل</a>
            </div>
            ...
            <div class="flex items-center gap-6 relative">
                ...
                <div class="relative">
                    ...
                    <div id="noti-dropdown" class="hidden absolute left-0 mt-3.5 w-80 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/60 rounded-2xl shadow-xl z-50 overflow-hidden text-right text-gray-800 dark:text-gray-200 animate-fadeIn">
                        ...
                        <div id="noti-list-container" class="max-h-64 overflow-y-auto divide-y divide-gray-50 dark:divide-gray-700/40"></div>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div id="settings-dropdown" class="hidden absolute left-0 mt-3.5 w-64 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-xl z-50 overflow-hidden text-right text-gray-800 dark:text-gray-200 animate-fadeIn">
                        ...
                        <div class="p-2 flex flex-col gap-1 text-xs font-semibold">
                            <button type="button" onclick="toggleTheme()" class="w-full flex items-center justify-between p-2.5 rounded-xl hover:bg-blue-50/40 dark:hover:bg-gray-700/40 text-gray-600 dark:text-gray-300 transition duration-150">
    ...
    <script>
        ...
        function renderNotificationsUI() {
            ...
            const dismissedKeys = JSON.parse(localStorage.getItem('dismissed_manager_notifications_keys')) || [];
            
            const activeNotifications = rawNotifications.filter(noti => {
                const notiKey = noti.consultation_id + '_manager_' + noti.status + '_' + new Date(noti.updated_at).getTime()/1000;
            ...
            if (displayLimitList.length > 0) {
                ...
                displayLimitList.forEach(noti => {
                    const notiKey = noti.consultation_id + '_manager_' + noti.status + '_' + new Date(noti.updated_at).getTime()/1000;
                    const isPendingApproval = (noti.status === 'بانتظار الاعتماد');
                    
                    let iconStyle = isPendingApproval ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 border-emerald-100/30' : 'bg-amber-50 dark:bg-amber-950/40 text-amber-500 border-amber-100/30';
                    let iconFas = isPendingApproval ? 'fa-file-signature' : 'fa-folder-open';
                    let headline = isPendingApproval ? 'قام المستشار برفع رد جديد بانتظار اعتمادك النهائي:' : 'استشارة قانونية جديدة واردة وبانتظار الإسناد والتوجيه:';
                    let linkUrl = isPendingApproval ? "{{ route('manager.consultations.incoming') }}" : "{{ route('manager.tasks') }}";

                    container.innerHTML += `
                        <div id="manager-noti-item-${noti.consultation_id}" class="flex items-center justify-between p-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition duration-150 group animate-fadeIn">
                            <a href="${linkUrl}" class="flex gap-3 items-start flex-1 min-w-0">
                                <div class="w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 border ${iconStyle}"><i class="fas ${iconFas}"></i></div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-[11px] font-bold text-gray-700 dark:text-gray-200 leading-snug">${headline}</p>
                                    <p class="text-[10px] text-gray-400 mt-0.5 truncate font-medium">"${noti.title}"</p>
                                </div>
                            </a>
                            <button type="button" onclick="dismissManagerNotification(event, '${noti.consultation_id}', '${notiKey}')" class="mr-2 text-gray-400 hover:text-red-500 p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition cursor-pointer flex items-center justify-center w-6 h-6 flex-shrink-0 opacity-0 group-hover:opacity-100 focus:opacity-100"><i class="fas fa-times text-xs"></i></button>
                        </div>`;
                });
            } else {
                ...
                container.innerHTML = `
                    <div id="empty-noti-view" class="p-6 text-center text-gray-400 dark:text-gray-500">
                        <i class="fas fa-check-double text-xl block mb-1.5 opacity-60"></i>
                        <p class="text-[11px] font-semibold">لا توجد إشعارات</p>
                    </div>`;
            }
        }

        function dismissManagerNotification(event, id, notiKey) {
            if (event) { event.preventDefault(); event.stopPropagation(); }
            let dismissedKeys = JSON.parse(localStorage.getItem('dismissed_manager_notifications_keys')) || [];
            if (!dismissedKeys.includes(notiKey)) {
                dismissedKeys.push(notiKey);
                localStorage.setItem('dismissed_manager_notifications_keys', JSON.stringify(dismissedKeys));
            }
        ...
        function clearAllNotifications() {
            ...
            rawNotifications.forEach(noti => {
                const notiKey = noti.consultation_id + '_manager_' + noti.status + '_' + new Date(noti.updated_at).getTime()/1000;
                if (!dismissedKeys.includes(notiKey)) { dismissedKeys.push(notiKey); }
            });
            localStorage.setItem('dismissed_manager_notifications_keys', JSON.stringify(dismissedKeys));
            renderNotificationsUI();
        }

        document.addEventListener("DOMContentLoaded", function() { renderNotificationsUI(); });
        ...
        window.onclick = function(event) {
            ...
            if (notiDropdown && !notiDropdown.classList.contains('hidden') && !document.getElementById('noti-btn').contains(event.target) && !notiDropdown.contains(event.target)) { notiDropdown.classList.add('hidden'); }
            if (settingsDropdown && !settingsDropdown.classList.contains('hidden') && !document.getElementById('settings-btn').contains(event.target) && !settingsDropdown.contains(event.target)) { settingsDropdown.classList.add('hidden'); }
        }
    </script>
    @stack('scripts')
</body>
